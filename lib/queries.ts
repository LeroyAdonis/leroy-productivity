...
export async function listTasks(userId = 1) {
  const res = await db.execute(sql<
    { id: number; title: string; estimated_minutes?: number; state?: 'now' | 'later' | 'done'; project_tag?: string; sort_order: number }
  >`
...
/** Create a new task */
export async function createTask(userId: number, title: string, estimated_minutes = 2, state: 'now' | 'later' = 'now', project_tag?: string) {
  const res = await db.execute(sql<
    { id: number; title: string; estimated_minutes: number; state: 'now' | 'later'; project_tag?: string }
  >`
...
export async function moveTask(taskId: number, newState: 'now' | 'later' | 'done', userId = 1) {
...
export async function completeTask(taskId: number, userId = 1) {
  await db.execute(sql`
    UPDATE tasks 
...
export async function listRoutineSteps(userId = 1) {
  const res = await db.execute(sql<
    { id: number; text: string; estimated_minutes: number; completed: boolean }
  >`
...
/** List all reminders */
export async function listReminders(userId = 1) {
  const res = await db.execute(sql<
    { id: number; message: string; action?: string | null; remind_at: string; state: string; estimated_minutes?: number; snooze_until?: string | null }
  >`
    SELECT id, message, action, remind_at, state, snooze_until
    FROM reminders
    WHERE user_id = ${userId}
...
export async function moveStep(stepId: number, newSortOrder: number, userId = 1) {
  await db.execute(sql`
...
export async function completeStep(stepId: number, userId = 1) {
  await db.execute(sql`
    UPDATE routine_steps 
